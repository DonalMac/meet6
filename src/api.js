  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const results = await axios.get(
        "https://hx8otqumxe.execute-api.eu-west-2.amazonaws.com/dev/api/get-auth-url");
...
const getToken = async (code) => {
  ...
  const { access_token } = await fetch('https://hx8otqumxe.execute-api.eu-west-2.amazonaws.com/dev/api/token' + '/' + encodeCode)
